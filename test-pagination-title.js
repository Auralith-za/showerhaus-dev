...
async function run() {
  const res1 = await fetch('https://ueicbp-za.myshopify.com/api/2024-01/graphql.json', {
    ...
    body: JSON.stringify({ query, variables: { first: 12, sortKey: "TITLE" } })
  });
  ...
  const res2 = await fetch('https://ueicbp-za.myshopify.com/api/2024-01/graphql.json', {
    ...
    body: JSON.stringify({ query, variables: { first: 12, after: cursor, sortKey: "TITLE" } })
  });
  ...
  console.log('Duplicates with TITLE:', overlap.length);
}
run();
